 ...
 * 
 * React component for exporting content to PDF format.
 * Features:
...
import { PDFExporter } from '../../../server/lib/pdf-exporter.js';
import type { PDFExportOptions, PDFExportProgress } from '../../../types/pdf-export.js';
...
export interface PDFExportButtonProps {
  ...
  onExportComplete?: (result: { success: boolean; size?: number; pageCount?: number }) => void;
...
export function PDFExportButton({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Export and download
      await exporter.downloadPDF(content, filename);
  ...
  return (
    <div className="relative">
      <button
        ...
        title={errorMessage || 'Export content as PDF'}
      >
        {getIcon()}
        <span>{getLabel()}</span>
...
/**
 * Compact PDF Export Button (icon only)
 */
export function PDFExportButtonCompact({
  content,
  filename = 'document.pdf',
  options = {},
  theme = 'light',
  ...
  onExportComplete,
  onExportError,
}: Omit<PDFExportButtonProps, 'label' | 'size' | 'variant' | 'showProgress'>) {
  const [isExporting, setIsExporting] = useState(false);
  const [exportStatus, setExportStatus] = useState<'idle' | 'success' | 'error'>('idle');
